...
For normal operation, the PC monitoring program is configured to run as a **Python `.pyw` application** and is added to **Windows Startup**.

Because of this, the monitoring program starts automatically when Windows starts. There is no need to manually open Command Prompt or run the monitoring script every time the PC is powered on.
...
Windows Startup
      │
      ▼
Python .pyw starts in the background
      │
      ├── CPU Usage  → psutil
      ├── GPU Usage  → NVIDIA NVML
      ├── FPS        → RTSS
      └── System Time
      │
      ▼
...
16×2 I2C LCD
      │
      ▼
Live CPU | GPU | FPS | Time
```

### Why `.pyw`?

The monitoring application uses the `.pyw` extension so that it can run in the background without opening a visible Command Prompt window.
...
Windows starts
     ↓
.pyw launches automatically
     ↓
PC statistics are collected
     ↓
Data is sent to ESP8266 every second
     ↓
LCD updates automatically
```

The goal is a **plug-and-monitor experience**: after the initial configuration, the user does not need to manually start the monitoring software.

> **Architecture graphic:** A visual version of the above flow can be added to this section to show the complete automatic startup and data path.
...
## 5. Automatic PC Monitoring

The monitoring program is normally configured as a **`.pyw` file in Windows Startup**, so it launches automatically when Windows starts.

For testing or troubleshooting, it can also be started manually:

```bash
python pc_to_lcd_optimized.py
```

The program polls the PC once per second and sends updated values to the ESP8266.
